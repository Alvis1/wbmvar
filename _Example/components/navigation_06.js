// Navigation 05 - OPTIMIZED Combined Teleport Component
...
AFRAME.registerComponent("cursor-teleport-01", {
  schema: {
  ...
  init() {
    ...
    if (this.debugMode)
      console.log("navigation-05: Initializing cursor-teleport component");
    ...
    // Set up VR cursor if in VR mode
    if (this.isVR) {
      this.setupVRCursor();
    }
  },
  ...
  createTeleportIndicator() {
    ...
    if (this.debugMode)
      console.log("navigation-05: Teleport indicator created");
  },
  ...
  setupVRSessionListeners() {
    ...
    document.addEventListener("visibilitychange", () => {
      if (
        !document.hidden &&
        this.isVR &&
        (!this.vrCursor || !this.vrRaycaster)
      ) {
        if (this.debugMode)
          console.log("navigation-05: Re-initializing VR cursor on tab focus");
        this.setupVRCursor();
      }
    });
  },
  ...
  initializeTunnelForVR() {
    ...
    if (this.tunnelElement) {
      ...
      if (this.debugMode)
        console.log("navigation-05: Tunnel initialized for VR");
    }
  ...
  setupVRCursor() {
    // Optimized VR cursor setup with immediate check and single retry
    this.vrCursor = this.el.sceneEl.querySelector("a-cursor");

    if (
      this.vrCursor &&
      this.vrCursor.components.raycaster &&
      this.vrCursor.components.raycaster.raycaster
    ) {
      this.vrRaycaster = this.vrCursor.components.raycaster;

      // Remove existing listener to prevent duplicates
      if (this.vrCursorClickHandler) {
        this.vrCursor.removeEventListener("click", this.vrCursorClickHandler);
      }

      // Create optimized click handler
      this.vrCursorClickHandler = (event) => {
        if (this.debugMode) console.log("navigation-05: VR cursor click");

        const teleportData = this.getTeleportPositionVR();
        if (teleportData) {
          this.teleportIndicator.position.copy(teleportData.point);

          let targetQuaternion = null;
          if (this.data.alignToSurface && teleportData.normal) {
            this.teleportQuaternion.setFromUnitVectors(
              this.upVector,
              teleportData.normal
            );
      ...
      this.vrCursor.addEventListener("click", this.vrCursorClickHandler);
      this.vrCursorInitialized = true;

      if (this.debugMode)
        console.log("navigation-05: VR cursor setup completed");
    } else {
      // Single retry with short delay
      if (!this.vrSetupRetried) {
        this.vrSetupRetried = true;
        setTimeout(() => this.setupVRCursor(), 200);
      } else if (this.debugMode) {
        console.warn("navigation-05: VR cursor setup failed after retry");
      }
    }
  },
  ...
  updateRaycastObjects() {
    ...
    if (this.data.collisionEntities !== "") {
      ...
      if (
        this.debugMode &&
        collisionEntities.length !== this.lastCollisionCount
      ) {
        console.log(
          "navigation-05: Collision entities count:",
          collisionEntities.length
        );
      }
      ...
      if (!this.collisionMesh) {
        ...
        if (this.debugMode)
          console.log("navigation-05: Created default collision plane");
      }
    ...
    if (this.rayCastObjects.length === 0 && previousCount > 0) {
      console.warn(
        "navigation-05: CRITICAL - Lost all collision objects! VR teleportation disabled."
      );
    }
  ...
  getTeleportPositionVR() {
    // Optimized VR teleport position calculation
    if (!this.vrRaycaster?.raycaster) {
      // Single attempt to re-establish connection
      if (this.vrCursor?.components.raycaster?.raycaster) {
        this.vrRaycaster = this.vrCursor.components.raycaster;
      } else {
        return false;
      }
    ...
    const intersects = this.vrRaycaster.raycaster.intersectObjects(
      this.rayCastObjects
    );
    ...
    if (
      !this.isValidNormalsAngle(intersect.face.normal, intersect.object) ||
    ...
    this.collisionObjectNormalMatrix.getNormalMatrix(
      intersect.object.matrixWorld
    );
    const worldNormal = this.tempVector3
      .copy(intersect.face.normal)
      .applyMatrix3(this.collisionObjectNormalMatrix)
      .normalize();
  ...
  getTeleportPosition() {
    ...
    return (x, y) => {
      if (this.rayCastObjects.length === 0 || !this.cam || !this.canvas)
        return false;
  ...
  isValidNormalsAngle(normal, object) {
    ...
    this.collisionWorldNormal
      .copy(normal)
      .applyNormalMatrix(this.collisionObjectNormalMatrix);
  ...
  transition(positionData, targetQuaternion) {
    ...
    if (targetQuaternion) {
      ...
      this.transitionRotEnd.setFromUnitVectors(
        this.upVector,
  ...
  mouseUp(event) {
    ...
    if (
      this.mousePosition.x === this.mouseOriginalPosition.x &&
      this.mousePosition.y === this.mouseOriginalPosition.y
    ) {
      const teleportData = this.getTeleportPosition()(
  ...
  tick(time, deltaTime) {
    ...
    if (this.debugMode && this.perfStats) {
      this.perfStats.avgFrameTime =
        this.perfStats.avgFrameTime * 0.9 + deltaTime * 0.1;
    ...
    if (!this.lastRaycastUpdate || time - this.lastRaycastUpdate > 15000) {
      ...
      if (this.debugMode && this.rayCastObjects.length !== previousCount) {
        console.log(
          "navigation-05: Raycast objects updated - count:",
          this.rayCastObjects.length
        );
      }
    ...
    if (!this.transitioning) {
      ...
      if (time - this.lastTeleportCheck > this.teleportCheckInterval) {
        if (this.isVR) {
          teleportPos = this.getTeleportPositionVR();
          if (this.debugMode && this.perfStats) this.perfStats.raycastCount++;
        } else if (!this.mobile) {
          teleportPos = this.getTeleportPosition()(
            this.mousePosition.x,
            this.mousePosition.y
          );
          if (this.debugMode && this.perfStats) this.perfStats.raycastCount++;
      ...
      if (teleportPos) {
        ...
        if (this.data.alignToSurface && teleportPos.normal) {
          const offsetPosition = this.tempVector3_2
            .copy(position)
            .add(
              this.tempVector3.copy(teleportPos.normal).multiplyScalar(0.01)
            );
...
AFRAME.registerComponent("pinch-teleport-02", {
  ...
  init() {
    this.debugMode = window.location.search.includes("debug=true");
    if (this.debugMode)
      console.log("navigation-05: Initializing optimized pinch-teleport");
    ...
    events.forEach((event) =>
      this.el.addEventListener(event, this.onPinchStarted)
    );
    endEvents.forEach((event) =>
      this.el.addEventListener(event, this.onPinchEnded)
    );
  },
  ...
  updateRaycastObjects() {
    ...
    if (this.data.collisionEntities) {
      ...
      if (
        ...
        console.log(
          "navigation-05: Pinch-teleport collision entities:",
          collisionEntities.length
        );
        this.lastCollisionCount = collisionEntities.length;
      }
    ...
    if (
      this.debugMode &&
      this.rayCastObjects.length !== this.lastRaycastCount
    ) {
      console.log(
        "navigation-05: Pinch-teleport raycast objects:",
        this.rayCastObjects.length
      );
      this.lastRaycastCount = this.rayCastObjects.length;
    }
  ...
  onPinchStarted(event) {
    if (this.debugMode)
      console.log("navigation-05: Pinch started:", event.type);
  ...
  teleportToCameraRig(position) {
    ...
    const animate = () => {
      ...
      this.data.cameraRig.object3D.position.lerpVectors(
        startPos,
        endPos,
        easedProgress
      );
  ...
  remove() {
    ...
    events.forEach((event) =>
      this.el.removeEventListener(event, this.onPinchStarted)
    );
    events.forEach((event) =>
      this.el.removeEventListener(event, this.onPinchEnded)
    );
  },
});
...
AFRAME.registerComponent("click-teleport-03", {
  ...
  init() {
    ...
    this.onClick = (event) => {
      if (this.debugMode)
        console.log("navigation-05: Click-teleport activated");
...
AFRAME.registerComponent("go-to", {
  ...
  moveCamera: function () {
    ...
    const hasRotation =
      Math.abs(this.data.rotation.x) > 0.001 ||
    ...
    this.rigEl.addEventListener("animationcomplete__go", this.onAnimEnd, {
      once: true,
    });
  },
...
AFRAME.registerComponent("save-position-and-rotation", {
  ...
  init: function () {
    this.debugMode = window.location.search.includes("debug=true");
    if (this.debugMode)
      console.log("navigation-05: save-position-and-rotation initialized");
    ...
    const recordPositionAndRotation = () => {
      try {
        ...
        if (this.debugMode)
          console.warn("Failed to save position/rotation:", e);
      }
    };
...
const TeleportDiagnostic = {
  run() {
    ...
    if (cameraRig?.components["cursor-teleport-01"]) {
      const comp = cameraRig.components["cursor-teleport-01"];
      console.log(
        "Teleport Component - VR:",
        comp.isVR,
        "| Objects:",
        comp.rayCastObjects.length,
        "| Ready:",
        comp.vrCursorInitialized
      );
    }
